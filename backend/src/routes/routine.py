...
def get_all_routines(
    ...
    routines = db.fetch_all(
        ...
        (current_user["id"],)  # 👈 missing comma, needs to be a tuple
    )
...
def get_routine(
    ...
    routine = db.fetch_one(
        ...
        (routine_id, current_user["id"])  # ✅ already correct
    )
    if not routine:
...
def create_routine(
    ...
    existing = db.fetch_one(
        "SELECT id FROM routine WHERE name = ? AND user_id = ?",
...
def update_routine(
    ...
    values = list(fields.values()) + [routine_id, current_user["id"]] 

    db.execute(
        f"UPDATE routine SET {set_clause} WHERE id = ? AND user_id = ?",
        tuple(values)  # 👈 wrap in tuple()
    )
