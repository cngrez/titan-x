...
router = APIRouter(prefix="/workout-sessions", tags=["workout_session"])

#GET /api/workout-sessions — anyone logged in can see all workout sessions
@router.get("/", response_model=list[WorkoutSessionResponse])
def get_all_workout_session(
    current_user=Depends(get_current_user),
    db: WorkoutDatabase = Depends(get_db)
):
    workout_session = db.fetch_all("SELECT * FROM workout_session WHERE user_id = ?", (current_user["id"],))
    return [dict(ws) for ws in workout_session]
...
def create_workout_session(
  ...
  cursor = db.execute(
        "INSERT INTO workout_session (date, notes, user_id, routine_id) VALUES (?, ?, ?, ?)",
        (body.date, body.notes, current_user["id"], body.routine_id)
  )
  workout_session = db.fetch_one(
        "SELECT * FROM workout_session WHERE id = ?", (cursor.lastrowid,)
  )
  return dict(workout_session)
...
def update_workout_session(
    ...
    existing = db.fetch_one(
        "SELECT * FROM workout_session WHERE id = ? AND user_id = ?", (workout_session_id, current_user["id"])
    )
    if not existing:
        raise HTTPException(status_code=404, detail="Workout session not found")
    ...
    db.execute(
        f"UPDATE workout_session SET {set_clause} WHERE id = ? AND user_id = ?",
        tuple(values + [current_user["id"]])
    )
    updated_workout_session = db.fetch_one(
        "SELECT * FROM workout_session WHERE id = ?", (workout_session_id,)
    )
...
def delete_workout_session(
    ...
    db.execute(
        "DELETE FROM workout_session WHERE id = ? AND user_id = ?", (workout_session_id, current_user["id"])
    )
